...
// Catálogo central de episódios por módulo (moduloId -> episódios).
const episodiosPorModulo={
  "pacotes-modulos":{
    moduloId:"pacotes-modulos",
    titulo:"Pacotes e Módulos",
    descricaoBase:"Nesta temporada estão os packs mais usados.",
    episodios:Array.from({length:10}).map((_,i)=>({
      id:`pacotes-modulos_ep${i+1}`,
      titulo:`Episódio ${i+1}`,
      descricao:`${i+1} • Módulo essencial para destravar resultados.`,
      thumb:episodiosPacotesModulosImgs[i%episodiosPacotesModulosImgs.length],
      video:episodiosPacotesModulosVideos[i%episodiosPacotesModulosVideos.length]
    }))
  },
  "so-na-arv":{
    moduloId:"so-na-arv",
    titulo:"Só na ARV",
    descricaoBase:"Seleção exclusiva com conteúdos inéditos.",
    episodios:Array.from({length:10}).map((_,i)=>({
      id:`so-na-arv_ep${i+1}`,
      titulo:`Episódio ${i+1}`,
      descricao:`${i+1} • Conteúdo premium e original.`,
      thumb:baseFilmesImgs[i%baseFilmesImgs.length],
      video:episodiosAvancadosVideos[i%episodiosAvancadosVideos.length]
    }))
  },
  "documentarios-mentalidade":{
    moduloId:"documentarios-mentalidade",
    titulo:"Documentários de mentalidade",
    descricaoBase:"Lei da atração e mentalidade vencedora.",
    episodios:Array.from({length:10}).map((_,i)=>({
      id:`documentarios-mentalidade_ep${i+1}`,
      titulo:`Episódio ${i+1}`,
      descricao:`${i+1} • Insight para elevar o nível.`,
      thumb:episodiosDocumentariosImgs[i%episodiosDocumentariosImgs.length],
      video:episodiosDocumentariosVideos[i%episodiosDocumentariosVideos.length]
    }))
  },
  "modulos-avancados":{
    moduloId:"modulos-avancados",
    titulo:"Módulos avançados",
    descricaoBase:"Bem-vindo ao próximo passo.",
    episodios:Array.from({length:10}).map((_,i)=>({
      id:`modulos-avancados_ep${i+1}`,
      titulo:`Capítulo ${i+1}`,
      descricao:`${i+1} • Estratégias avançadas aplicadas.`,
      thumb:episodiosAvancadosImgs[i%episodiosAvancadosImgs.length],
      video:episodiosAvancadosVideos[i%episodiosAvancadosVideos.length]
    }))
  },
  "inicio":{
    moduloId:"inicio",
    titulo:"Início",
    descricaoBase:"Atualizações da semana para você continuar.",
    episodios:Array.from({length:10}).map((_,i)=>({
      id:`inicio_ep${i+1}`,
      titulo:`Destaque ${i+1}`,
      descricao:`${i+1} • Conteúdos selecionados para hoje.`,
      thumb:baseAnimesImgs[i%baseAnimesImgs.length],
      video:episodiosDocumentariosVideos[i%episodiosDocumentariosVideos.length]
    }))
  }
};
...
function renderHome(){
  ...
  content.innerHTML="";
  const sections=[
    {moduloId:"pacotes-modulos",titulo:"Pacotes e Módulos",tipo:"Modalidade",imagens:imagensSeries},
    {moduloId:"so-na-arv",titulo:"Só na ARV",tipo:"packs",imagens:imagensFilmes},
    {moduloId:"documentarios-mentalidade",titulo:"Documentários de mentalidade",tipo:"Modalidade",imagens:imagensAnimes},
    {moduloId:"modulos-avancados",titulo:"Módulos avançados",tipo:"categorias",imagens:imagensFilmes},
    {moduloId:"inicio",titulo:"Inicio",tipo:"inicio",imagens:imagensAnimes}
  ];
...
// Recebe moduloId e carrega dados específicos do módulo selecionado.
function openEpisodes(moduloId="pacotes-modulos") {
  const content = document.getElementById("content");
  const sectionData = episodiosPorModulo[moduloId] || episodiosPorModulo["pacotes-modulos"];

  content.innerHTML = `
    <div class="section">
      <div class="section-title">${sectionData.titulo}</div>
    </div>
  `;

  sectionData.episodios.forEach((episode,idx)=>{
    const episodeId = episode.id;
    const watched = localStorage.getItem("watched_" + episodeId);

    content.innerHTML += `
      <div class="episode" onclick="openPlayerWithEpisode('${episodeId}','${episode.video}','${episode.thumb}')">
        <video muted playsinline preload="metadata" poster="${episode.thumb}">
          <source src="${episode.video}" type="video/mp4">
        </video>
        <div>
          <h4>${episode.titulo}</h4>
          <p>${sectionData.descricaoBase} ${episode.descricao || idx+1}</p>
